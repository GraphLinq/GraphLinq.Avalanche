...
{
    [NodeDefinition("AvaxConnection", "Avalanche Connector", NodeTypeEnum.Connector, "Blockchain.Avalanche")]
    [NodeGraphDescription("Connection to the Avalanche network, can be used as Managed connection (without in parameters) or with your own node")]
    public class AvaxConnection : Node
    {
        public AvaxConnection(string id, BlockGraph graph)
            : base(id, graph, typeof(AvaxConnection).Name)
        {
            ...
            this.OutParameters.Add("connection", new NodeParameter(this, "connection", typeof(AvaxConnection), true));
        }
        ...
        public override void SetupConnector()
        {
            if(this.InParameters["url"].GetValue() == null || this.InParameters["socketUrl"].GetValue() == null)
            {
                ...
                this.Web3Client = Plugin.Web3ClientAVAX;
                this.SocketClient = Plugin.SocketClientAVAX;
        ...
        public Web3 InstanciateWeb3Account(Account account)
        {
            if (this.InParameters["url"].GetValue() == null || this.InParameters["socketUrl"].GetValue() == null)
            {
                return new Web3(account, Plugin.Web3ClientAVAX.Client);
            }
            else
            {
